    ...
    <div class="wrap sb-seo-issues-page">
        ...
        <div class="sb-sitewide-section" style="margin-bottom: 30px;">
            ...
            <div class="sb-sitewide-stats" style="background: #fff; border: 1px solid #c3c4c7; border-radius: 4px; margin: 15px 0;">
                <h3 class="sb-collapsible-header" style="margin: 0; border-bottom: 1px solid #c3c4c7;">
                    ...
                    <?php printf(
                        __('%d total issues (%d errors, %d warnings, %d good practices)', 'seo-booster'),
        ...
        <div class="notice seobooster-notice notice-info">
            <p>
                ...
                <?php printf(
                    __('%d total possibilities (%d critical, %d high, %d medium) • %d URLs analyzed', 'seo-booster'),
                    $stats['total_issues'],
                    $stats['critical'],
                    $stats['high'],
                    $stats['medium'],
...
/**
 * Handle bulk actions for the possibilities table.
 *
 * @since 6.1.26
 * @return void
 */
function handle_bulk_actions()
{
    ...
    $possibility_ids = isset($_POST['possibility_ids']) ? array_map('intval', $_POST['possibility_ids']) : (isset($_POST['issue_ids']) ? array_map('intval', $_POST['issue_ids']) : []);
    if (empty($possibility_ids)) {
        add_action('admin_notices', function() {
            echo '<div class="notice notice-error"><p>' . __('No possibilities selected.', 'seo-booster') . '</p></div>';
        });
        return;
    }
    $status_mapping = [
    ...
    if (!isset($status_mapping[$action])) {
        add_action('admin_notices', function() {
            echo '<div class="notice notice-error"><p>' . __('Invalid action.', 'seo-booster') . '</p></div>';
        });
        return;
    }
    ...
    if ($result) {
        ...
        add_action('admin_notices', function() use ($count) {
            echo '<div class="notice notice-success"><p>' . sprintf(__('%d possibility(ies) updated successfully.', 'seo-booster'), $count) . '</p></div>';
        });
    } else {
        add_action('admin_notices', function() {
            echo '<div class="notice notice-error"><p>' . __('Failed to update possibilities.', 'seo-booster') . '</p></div>';
        });
    }
}

// Handle bulk actions if this is a POST request
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['page']) && $_POST['page'] === 'sb2_seo_issues') {
    handle_bulk_actions();
}
