...
class SB_SEO_Metabox
{
    ...
    {
        add_action('add_meta_boxes', [__CLASS__, 'add_seo_metabox']);
        add_action('admin_enqueue_scripts', [__CLASS__, 'enqueue_scripts']);
        add_action('wp_ajax_sb_seo_get_keyword_suggestions', [__CLASS__, 'get_keyword_suggestions']);
        ...
        add_action('wp_ajax_sb_seo_analysis', [__CLASS__, 'ajax_seo_analysis']);
        add_action('wp_ajax_sb_seo_download_and_analyze', [__CLASS__, 'ajax_download_and_analyze']);
        add_action('wp_ajax_sb_seo_get_saved_analysis', [__CLASS__, 'ajax_get_saved_analysis']);
        add_action('wp_ajax_sb_seo_generate_wp_connector_suggestions', [__CLASS__, 'ajax_generate_wp_connector_suggestions']);
        add_action('wp_ajax_sb_seo_generate_credits_suggestions', [__CLASS__, 'ajax_generate_credits_suggestions']);
        add_action('wp_ajax_sb_seo_generate_product_seo', [__CLASS__, 'ajax_generate_product_seo']);
        add_action('wp_ajax_sb_seo_generate_comprehensive_analysis', [__CLASS__, 'ajax_generate_comprehensive_analysis']);
        add_action('wp_ajax_sb_seo_poll_credits_request', [__CLASS__, 'ajax_poll_credits_request']);
        add_action('wp_ajax_sb_seo_save_pending_request', [__CLASS__, 'ajax_save_pending_request']);
        add_action('wp_ajax_sb_seo_clear_pending_request', [__CLASS__, 'ajax_clear_pending_request']);
        add_action('wp_ajax_sb_seo_get_recent_request_statuses', [__CLASS__, 'ajax_get_recent_request_statuses']);
        add_action('wp_ajax_sb_seo_retry_request', [__CLASS__, 'ajax_retry_request']);
        add_action('wp_ajax_sb_seo_get_llm_suggestions', [__CLASS__, 'ajax_get_llm_suggestions']);
        add_action('wp_ajax_sb_seo_analyze_step', [__CLASS__, 'ajax_analyze_step']);
        add_action('wp_ajax_sb_seo_toggle_analysis_exclusion', [__CLASS__, 'ajax_toggle_analysis_exclusion']);

        // Save exclusion preference on post save
        add_action('save_post', [__CLASS__, 'save_exclusion_preference']);

        // Add taxonomy meta boxes
        add_action('category_edit_form', [__CLASS__, 'render_taxonomy_metabox'], 10, 2);
        add_action('post_tag_edit_form', [__CLASS__, 'render_taxonomy_metabox'], 10, 2);
    }
        ...
        if (!$is_term && $item_type === 'post') {
            ...
            if ($is_auto_excluded) {
                ...
                if ($post_status === 'private') {
                    ...
                    $wc_page_types = ['cart' => __('Cart', 'seo-booster'), 'checkout' => __('Checkout', 'seo-booster'), 'myaccount' => __('My Account', 'seo-booster'), 'shop' => __('Shop', 'seo-booster')];
...
?>
        <div id="sb-seo-metabox" class="sb-seo-metabox">
            <!-- Exclusion Checkbox -->
            ...
            <div class="sb-seo-analysis-section">
                    ...
                    <div class="sb-analysis-info">
                        ...
                        if ($last_download) {
                            ...
                            echo sprintf(
                                ...
                                date('Y-m-d H:i:s', $last_download)
                            );
                            echo '</small><br>';
                        } else {
                ...
                <div id="sb-recent-requests-wrap" class="sb-recent-requests-wrap">
                ...
                <p class="sb-ai-suggestions-label"><?php esc_html_e('AI suggestions', 'seo-booster'); ?></p>
                <div id="sb-llm-suggestions-display">
                    <!-- Display area for title/description suggestions -->
                </div>
        ...
        <div id="sb-seo-metabox" class="sb-seo-metabox sb-seo-attachment-metabox">
            <div class="sb-seo-ai-section">
                ...
                <div class="sb-image-ai-generator">
                    ...
                    <!-- Recent requests for this image (same as post metabox) -->
                    <div id="sb-recent-requests-wrap" class="sb-attachment-recent-requests" style="margin-top: 15px; padding: 12px; background: #f6f7f7; border: 1px solid #c3c4c7; border-radius: 4px;">
                        <h4 style="margin: 0 0 10px 0; font-size: 13px;"><?php esc_html_e('Recent requests', 'seo-booster'); ?></h4>
                        <div id="sb-recent-requests-list"><span class="sb-recent-requests-placeholder" style="color:#646970;"><?php esc_html_e('Loading…', 'seo-booster'); ?></span></div>
                    </div>
...
<?php
    ...
    public static function enqueue_scripts($hook)
    {
        ...
        if (!in_array($hook, ['post.php', 'post-new.php', 'term.php', 'edit-tags.php', 'seo-booster_page_sb2_seo_settings'])) {
            return;
        }
        ...
        wp_enqueue_script(
            ...
            ['jquery', 'sb-seo-examples-display'],
        ...
        $recent_request_ids = $post_id ? get_post_meta($post_id, '_sb_recent_request_ids', true) : [];
        if (!is_array($recent_request_ids)) {
            $recent_request_ids = [];
        }
        $saved_comprehensive = $post_id ? get_post_meta($post_id, '_sb_comprehensive_analysis_result', true) : [];
        if (!is_array($saved_comprehensive) || empty($saved_comprehensive)) {
            $saved_comprehensive = null;
        }

        wp_localize_script('sb-seo-metabox', 'sb_seo_metabox', [
    ...
    public static function get_keyword_suggestions()
    {
        ...
        if (empty($keywords)) {
            wp_send_json_success(['keywords' => [], 'message' => __('No keyword data available for this URL', 'seo-booster')]);
        }
    ...
    public static function detect_seo_plugins()
    {
        ...
        // Check for SEOPress
        if (function_exists('seopress_get_service') || class_exists('SEOPress')) {
            $detected_plugins['seopress'] = [
                'name' => 'SEOPress',
                'function' => 'seopress_get_service'
            ];
        }

        // Check for All in One SEO
        if (class_exists('AIOSEO') || function_exists('aioseo')) {
            $detected_plugins['aioseo'] = [
                'name' => 'All in One SEO',
                'class' => 'AIOSEO'
            ];
        }
        ...
        if (class_exists('The_SEO_Framework') || function_exists('the_seo_framework')) {
            $detected_plugins['seoframework'] = [
    ...
    public static function ajax_seo_analysis()
    {
        ...
        if (!$object_id) {
            wp_send_json_error(['message' => __('Invalid object ID', 'seo-booster')]);
        }
        ...
        try {
            ...
            wp_send_json_error(['message' => __('Analysis failed', 'seo-booster')]);
        }
    }
    ...
    public static function ajax_download_and_analyze()
    {
        ...
        try {
            ...
            $results['last_download'] = $last_download ? date('Y-m-d H:i:s', $last_download) : null;

            wp_send_json_success($results);
        } catch (\Exception $e) {
            Utils::log('SEO Analysis Error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine(), 2);
            wp_send_json_error(['message' => __('Download and analysis failed: ' . $e->getMessage(), 'seo-booster')]);
        }
    }
    ...
    public static function ajax_get_saved_analysis()
    {
        ...
        try {
            ...
            if ($has_score) {
                ...
                if ($object_type === 'post') {
                    ...
                    if (!empty($last_download)) {
                        $results['last_download'] = date('Y-m-d H:i:s', $last_download);
                    }
                }
            ...
            wp_send_json_error(['message' => __('Failed to get analysis', 'seo-booster')]);
        }
    }
    ...
    public static function ajax_generate_wp_connector_suggestions()
    {
        ...
        if (!LLM_Helper::wp_ai_is_available()) {
            wp_send_json_error(['message' => LLM_Helper::wp_ai_unavailable_message()]);
        }
        ...
        if (!function_exists('wp_ai_client_prompt')) {
            wp_send_json_error(['message' => __('WordPress AI is not available. Use WordPress 7 or later and configure Settings → Connectors.', 'seo-booster')]);
        }
    ...
    public static function ajax_generate_credits_suggestions()
    {
        ...
        if (!Credits_Service::is_registered()) {
            wp_send_json_error(['message' => __('Credits account not connected. Please go to Settings to connect.', 'seo-booster')]);
        }
        ...
        try {
            ...
            $meta_description = '';
            $active_plugin = SEO_Plugin_Integration::get_active_seo_plugin();
            if ($active_plugin) {
    ...
    public static function ajax_generate_product_seo()
    {
        ...
        if (!Credits_Service::is_registered()) {
            wp_send_json_error(['message' => __('Credits account not connected.', 'seo-booster')]);
        }
                    ...
                    foreach ($attrs as $attr) {
                        if (is_a($attr, 'WC_Product_Attribute')) {
                            ...
                            $values = $attr->is_taxonomy()
                                ? implode(', ', wc_get_product_terms($post_id, $attr->get_name(), ['fields' => 'names']))
    ...
    public static function ajax_poll_credits_request()
    {
        ...
        if (empty($request_id)) {
            wp_send_json_error(['message' => __('Missing request ID', 'seo-booster')]);
        }
    ...
    public static function ajax_save_pending_request()
    {
        ...
        if (!$post_id || !$request_id) {
            wp_send_json_error(['message' => __('Invalid post or request ID', 'seo-booster')]);
        }
    ...
    public static function ajax_retry_request()
    {
        ...
        $request_id = isset($_POST['request_id']) ? sanitize_text_field(wp_unslash($_POST['request_id'])) : '';
        if (!$request_id) {
            wp_send_json_error(['message' => __('Invalid request ID', 'seo-booster')]);
        }
        $result = Credits_Service::retry_request($request_id);
        if ($result['success']) {
            wp_send_json_success(['enqueued' => true, 'request_id' => $request_id]);
        }
        wp_send_json_error(['message' => $result['error']]);
    }
    ...
    public static function ajax_get_llm_suggestions()
    {
        ...
        $response = $saved ?: ['titles' => null, 'descriptions' => null];
