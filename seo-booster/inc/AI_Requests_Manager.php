...
class AI_Requests_Manager
{
    ...
    public static function update_request($request_id, $response_data, $status = 'completed', $error_message = null)
    {
        ...
        $result = $wpdb->update($table_name, $update_data, ['id' => $request_id]);
    ...
    public static function process_request($request_id)
    {
        ...
        if (!$request_data) {
            self::update_request($request_id, [], 'failed', 'Invalid request data');
            return false;
        }
        ...
        if (empty($ai_endpoint_url)) {
            self::update_request($request_id, [], 'failed', 'AI endpoint URL not configured');
            return false;
        }

        // Example implementation (replace with your actual AI endpoint logic)
        $response = wp_remote_post($ai_endpoint_url, [
        ...
        if (is_wp_error($response)) {
            ...
            self::update_request($request_id, [], 'failed', $error_message);
            return false;
        }
        ...
        if ($response_code !== 200) {
            self::update_request($request_id, [], 'failed', "HTTP {$response_code}: {$response_body}");
            return false;
        }
        ...
        if (!$response_data) {
            self::update_request($request_id, [], 'failed', 'Invalid response from AI endpoint');
            return false;
        }
