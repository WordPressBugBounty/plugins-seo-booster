...
class Bulk_SEO_Analysis
{
    ...
    public static function init()
    {
        ...
        add_action('admin_init', [__CLASS__, 'add_bulk_actions_for_post_types']);
        
        // AJAX handlers
        add_action('wp_ajax_sb_get_bulk_seo_status', [__CLASS__, 'ajax_get_bulk_status']);
        add_action('wp_ajax_sb_process_bulk_seo_analysis', [__CLASS__, 'ajax_process_bulk_analysis']);
        add_action('wp_ajax_sb_cancel_bulk_seo_analysis', [__CLASS__, 'ajax_cancel_bulk_analysis']);
    ...
    public static function add_bulk_actions_for_post_types()
    {
        ...
        foreach ($post_types as $post_type) {
            ...
            add_filter("handle_bulk_actions-edit-{$post_type}", [__CLASS__, 'handle_bulk_action'], 10, 3);
        }
    }
    ...
    public static function ajax_process_bulk_analysis()
    {
        ...
        if (!$post) {
            ...
            wp_send_json_error(['message' => __('Post not found', 'seo-booster')]);
        }

        // Check if excluded from analysis
        if (SEO_Issues_Manager::should_exclude_from_analysis($post_id)) {
            self::update_batch_status($batch_id, 'failed', $post_id);
            wp_send_json_error(['message' => __('Post is excluded from analysis', 'seo-booster')]);
        }

        try {
            // Run SEO analysis with full page content
            $analysis = new SEO_Analysis($post_id, 'post');
            $results = $analysis->analyze(true, false); // true = use full page content, false = don't force redownload
    ...
    private static function update_batch_status($batch_id, $status, $post_id = 0)
    {
        ...
        if ($status === 'processed') {
            ...
            $batch_status['processing_ids'] = array_diff($batch_status['processing_ids'], [$post_id]);
    ...
    public static function enqueue_scripts($hook)
    {
        ...
        // Localize script
        wp_localize_script('sb-bulk-seo-status', 'sbBulkSeoStatus', [
            ...
            'strings' => [
                ...
                'processing_status' => __('Processing %d of %d posts...', 'seo-booster'),
