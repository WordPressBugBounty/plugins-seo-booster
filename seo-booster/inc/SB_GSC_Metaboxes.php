...
class SB_GSC_Metaboxes
{
    ...
    public static function init()
    {
        ...
        add_action('wp_ajax_sb_gsc_reanalyze_keywords', [__CLASS__, 'reanalyze_keywords']);
        add_action('wp_ajax_sb_gsc_load_libraries', [__CLASS__, 'load_libraries']);

        add_action( "product_cat_edit_form", [__CLASS__, 'render_taxonomy_metabox'], 9999, 2);
        add_action( "product_tag_edit_form", [__CLASS__, 'render_taxonomy_metabox'], 10, 2);
        ...
        foreach ($public_taxonomies as $taxonomy) {
            if ($taxonomy !== 'post_tag') { // Skip post_tag as we already added it above
                add_action("{$taxonomy}_edit_form", [__CLASS__, 'render_taxonomy_metabox'], 10, 2);
            }
        }
    }
    ...
    public static function enqueue_scripts($hook)
    {
        ...
        wp_enqueue_script('sb-gsc-metabox', SEOBOOSTER_PLUGINURL . 'js/sb-gsc-metabox.js', array('jquery'), $js_version, true);
    ...
    public static function reanalyze_keywords() {
        ...
        if (!$item_id && empty($public_url)) {
            wp_send_json_error(['message' => __('Invalid ID or URL', 'seo-booster')]);
        }
        ...
        if (empty($public_url)) {
            wp_send_json_error(['message' => __('Failed to get URL for the item', 'seo-booster')]);
        }
        ...
        wp_send_json_success([
            'message' => sprintf(__('Successfully scheduled %2$d jobs for %3$s. Reloading in 4 seconds...', 'seo-booster'), 
                $deleted, 
                $scheduled_jobs, 
