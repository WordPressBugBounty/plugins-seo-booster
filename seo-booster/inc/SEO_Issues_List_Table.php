...
class SEO_Issues_List_Table extends \WP_List_Table
{
    ...
    {
        return [
            'mark_fixed' => __('Mark as Fixed', 'seo-booster'),
            'ignore_temp' => __('Ignore Temporarily', 'seo-booster'),
            'ignore_permanent' => __('Ignore Permanently', 'seo-booster'),
            'mark_active' => __('Mark as Active', 'seo-booster')
        ];
    }
    ...
    public function prepare_items()
    {
        ...
        $filters = [
            ...
            'issue_type' => isset($_GET['issue_type']) ? sanitize_text_field($_GET['issue_type']) : '',
        ...
        // Get sort parameters
        $orderby = isset($_GET['orderby']) ? sanitize_text_field($_GET['orderby']) : 'total_issues';
    ...
    private function get_items($filters, $per_page, $offset)
    {
        ...
        // Validate orderby
        $allowed_orderby = ['message', 'severity', 'user_status', 'created_at'];
        if (!in_array($orderby, $allowed_orderby)) {
            $orderby = 'severity';
        }
    ...
    public function column_issues($item)
    {
        ...
        $html .= sprintf('<strong>%d</strong>', $total);
        ...
        if (!empty($parts)) {
            $html .= '<div class="sb-issues-breakdown">';
            $html .= '(' . implode(', ', $parts) . ')';
            $html .= '</div>';
        }
    ...
    public function column_actions($item)
    {
        ...
        $html .= sprintf('<button type="button" class="button button-small sb-expand-url" data-url-id="%d">%s</button>', 
            $item->id, 
    ...
    private function display_filters()
    {
        ...
        if ($current_severity || $current_status || $current_search) {
            $clear_url = remove_query_arg(['severity', 'status', 's']);
