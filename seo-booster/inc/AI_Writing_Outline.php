...
class AI_Writing_Outline
{
    ...
    {
        add_action('wp_ajax_sb_seo_get_writing_outline', [__CLASS__, 'ajax_get_writing_outline']);
        add_action('wp_ajax_sb_seo_get_saved_outline', [__CLASS__, 'ajax_get_saved_outline']);
        add_action('wp_ajax_sb_seo_generate_article_from_outline', [__CLASS__, 'ajax_generate_article_from_outline']);
    }
    ...
    public static function ajax_get_writing_outline()
    {
        ...
        if (!$post_id || !$user_prompt) {
            wp_send_json_error(['message' => __('Invalid request', 'seo-booster')]);
        }
    ...
    private static function build_context($post_id)
    {
        ...
        foreach ($taxonomies as $tax) {
            ...
            if (!is_wp_error($terms) && !empty($terms)) {
                $context['taxonomies'][$tax] = array_map(function($t) {
                    return $t->name;
                }, $terms);
            }
        }
    ...
    private static function generate_outline($post_id, $user_prompt)
    {
        ...
        if ($ai_provider === 'wordpress') {
            ...
            $text = wp_ai_client_prompt($full_prompt)
    ...
    public static function ajax_get_saved_outline()
    {
        ...
        if (empty($saved)) {
            wp_send_json_success(['outline' => null]);
        }
    ...
    private static function generate_article($post_id, $outline, $language)
    {
        ...
        if ($provider === 'wordpress') {
            ...
            $response_text = wp_ai_client_prompt($user_prompt)
            ...
            return [ 'title' => sanitize_text_field($parsed['title']), 'content_html' => wp_kses_post($parsed['content_html']) ];
        }
