...
class AI_Image_Generator
{
    ...
    {
        add_action('wp_ajax_sb_seo_generate_image_descriptions', [__CLASS__, 'ajax_generate_image_descriptions']);
        add_action('wp_ajax_sb_seo_restore_image_content', [__CLASS__, 'ajax_restore_image_content']);
        add_action('wp_ajax_sb_seo_apply_image_content', [__CLASS__, 'ajax_apply_image_content']);
    }
    ...
    public static function ajax_generate_image_descriptions()
    {
        ...
        if (!$attachment_id) {
            wp_send_json_error(['message' => __('Invalid attachment ID', 'seo-booster')]);
        }
    ...
    public static function generate_descriptions($attachment_id)
    {
        ...
        update_post_meta($attachment_id, '_sb_ai_image_latest_content', [
    ...
    private static function get_attachment_context($attachment_id)
    {
        ...
        if ($parent_id) {
            ...
            if ($parent_post) {
                ...
                // Get categories
                $categories = wp_get_post_categories($parent_id, ['fields' => 'names']);
                if (!empty($categories)) {
    ...
    private static function store_previous_content($attachment_id)
    {
        ...
        if (!empty($current_title) || !empty($current_alt) || !empty($current_caption) || !empty($current_description)) {
            update_post_meta($attachment_id, '_sb_ai_image_previous_content', [
                'title' => $current_title,
