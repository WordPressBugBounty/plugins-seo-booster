...
class Media_Library_Enhancements
{
    ...
    {
        // Add Alt text column
        add_filter('manage_media_columns', [__CLASS__, 'add_alt_text_column']);
        add_action('manage_media_custom_column', [__CLASS__, 'display_alt_text_column'], 10, 2);
        ...
        // AJAX handlers
        add_action('wp_ajax_sb_get_bulk_status', [__CLASS__, 'ajax_get_bulk_status']);
        add_action('wp_ajax_sb_process_bulk_image', [__CLASS__, 'ajax_process_bulk_image']);
        add_action('wp_ajax_sb_cancel_bulk_generation', [__CLASS__, 'ajax_cancel_bulk_generation']);
    ...
    public static function add_bulk_action($actions)
    {
        ...
        if (in_array($ai_provider, ['wordpress', 'seobooster'], true) && AI_Image_Generator::model_supports_vision()) {
    ...
    public static function handle_bulk_action($redirect_to, $action, $post_ids)
    {
        ...
        $ai_provider = get_option('seobooster_ai_provider', 'disabled');
        if (!in_array($ai_provider, ['wordpress', 'seobooster'], true)) {
    ...
    public static function ajax_process_bulk_image()
    {
        ...
        if (!$attachment) {
            ...
            wp_send_json_error(['message' => __('Attachment not found', 'seo-booster')]);
        }

        // Check if it's an image
        if (!wp_attachment_is_image($attachment_id)) {
            self::update_batch_status($batch_id, 'failed', $attachment_id);
            wp_send_json_error(['message' => __('Not an image', 'seo-booster')]);
        }
    ...
    private static function update_batch_status($batch_id, $status, $attachment_id = 0)
    {
        ...
        if (!isset($batch_status['processed_ids'])) {
            $batch_status['processed_ids'] = [];
        }
        if (!isset($batch_status['failed_ids'])) {
            $batch_status['failed_ids'] = [];
        }
        if (!isset($batch_status['processing_ids'])) {
            $batch_status['processing_ids'] = [];
        }
        ...
        if ($status === 'processed') {
            ...
            $batch_status['processing_ids'] = array_diff($batch_status['processing_ids'], [$attachment_id]);
    ...
    public static function ajax_get_bulk_status()
    {
        ...
        if (!$completed && !$cancelled && isset($batch_status['attachment_ids']) && is_array($batch_status['attachment_ids'])) {
            $processed_ids = isset($batch_status['processed_ids']) ? $batch_status['processed_ids'] : [];
            $failed_ids = isset($batch_status['failed_ids']) ? $batch_status['failed_ids'] : [];
            $processing_ids = isset($batch_status['processing_ids']) ? $batch_status['processing_ids'] : [];
    ...
    public static function enqueue_scripts($hook)
    {
        ...
        // Localize script
        wp_localize_script('sb-media-bulk-status', 'sbBulkStatus', [
            ...
            'strings' => [
                ...
                'processing_status' => __('Processing %d of %d images...', 'seo-booster'),
