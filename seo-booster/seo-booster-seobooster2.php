...
if ( $latest_date ) {
    ...
    if ( $past_30_days_data && $previous_30_days_data ) {
        ...
        $overview_message .= sprintf(
            ...
            ( $position_change <= 0 ? '+' : '' ),
            // Note: For position, negative change is good
...
if ( $selected_site && 0 < $unique_days ) {
    ...
    // End of else block for $need_reauth
    ...
    if ( !empty( $top_possibilities ) || $possibilities_stats['total_issues'] > 0 ) {
        ...
        if ( $possibilities_stats['total_issues'] > 0 ) {
            ?>
                        <p class="quickie">
                            <?php 
            printf( esc_html__( 'We\'ve detected %1$s SEO possibilities across your site. Here are the most impactful ones to address:', 'seo-booster' ), '<strong>' . number_format_i18n( $possibilities_stats['total_issues'] ) . '</strong>' );
            ?>
                        </p>
                    <?php 
        }
        ...
        if ( !empty( $top_possibilities ) ) {
            ...
            ?>" class="button button-secondary">
                                <?php 
            esc_html_e( 'Go to SEO Possibilities', 'seo-booster' );
            ?>
                            </a>
                        </p>
                    <?php 
